...
| --- | --- | --- |
| **Vulnerability Scan** | [`.github/workflows/vulnerability-scan.yml`](.github/workflows/vulnerability-scan.yml) | Daily Trivy filesystem scan for CRITICAL findings; posts **one Slack message per finding** with description, why it's critical (CVSS score/vector) and the exploit path. |
| **Vulnerability Report** | [`.github/workflows/vulnerability-report.yml`](.github/workflows/vulnerability-report.yml) | Daily + weekly effectiveness report posted to Slack, computed by [`.github/scripts/vuln_report.py`](.github/scripts/vuln_report.py). |
